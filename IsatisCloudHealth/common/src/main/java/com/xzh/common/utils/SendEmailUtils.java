...
public class SendEmailUtils {//发送邮件的工具类
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail, String validateCode) throws Exception {
        ...
        // 4. Subject: 邮件主题
        message.setSubject("修改邮箱验证", "UTF-8");

        // 5. Content: 邮件正文（可以使用html标签）
        //特别需要注意的是如果邮件正文包含了"验证码"字段会被拦截
        message.setContent(validateCode, "text/html;charset=UTF-8");
        // 6. 设置发件时间
    ...
    public static void sendEmail4ValidateCode(String receiveMailAccount, String validateCode) throws Exception {
        ...
        */
        ...
        // 3. 创建一封邮件
        MimeMessage message = createMimeMessage(session, myEmailAccount, receiveMailAccount, validateCode);
